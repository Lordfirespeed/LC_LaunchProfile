...
proton-launch-pre-init() {
  ...
  export SteamAppId="3241660"
  export SteamGameId="3241660"
}
...
proton-launch-user-specific-init() {
  ...
  declare -gA CONFIG=(
    [windowCount]=1
    [windowSizePresetIndex]=0
    [monitorIndex]=0
    [profileDirectory]="${HOME}/.config/r2modmanPlus-local/REPO/profiles"
    [selectedProfileName]="Test Team Upgrades"
  )
}
...
proton-launch-post-init() {
  ...
  # this might help later; for now, don't really care
  # export WINEDLLOVERRIDES="winhttp.dll=n,b" 
}

game-prepare-launch-arguments() {
  local windowWidth="${1:?}" windowHeight="${2:?}"
  
  local resolutionSwitches=(-screen-fullscreen 0 -screen-width "${windowWidth}" -screen-height "${windowHeight}")
  local doorstopEnableSwitches=(--doorstop-enable true)
  local doorstopTargetFullPath="${CONFIG[profileDirectory]}/${CONFIG[selectedProfileName]}/BepInEx/core/BepInEx.Preloader.dll"
  local doorstopTargetSwitches=(--doorstop-target "${doorstopTargetFullPath}")

  ARGUMENTS=("${resolutionSwitches[@]}" "${doorstopEnableSwitches[@]}" "${doorstopTargetSwitches[@]}")
}
...
game-prepare-launch-arguments $1 $2

for i in $(seq 1 ${CONFIG[windowCount]});
do
    # Run the game
    python3 "${PROTON_EXEC}" run "${APP_EXEC_FULL_PATH}" "${ARGUMENTS[@]}" &>/dev/null & 
done
